...
ESX.ProgressBar = function(duration, message)
    exports["qb-taskbar"]:taskBar(duration, message, 0)
end
...
ESX.UI.Menu.Close = function(type, namespace, name)
	for i=1, #ESX.UI.Menu.Opened, 1 do
		if ESX.UI.Menu.Opened[i] then
			if ESX.UI.Menu.Opened[i].type == type and ESX.UI.Menu.Opened[i].namespace == namespace and ESX.UI.Menu.Opened[i].name == name then
				local source = source
				TriggerServerEvent("InteractSound_SV:PlayOnSource", "menu_back", 0.4)
							...
							if playersNearby[selectedPlayer] then
								...
								if IsPedOnFoot(selectedPlayerPed) and not IsPedFalling(selectedPlayerPed) then
									if type == 'item_weapon' then
										TriggerServerEvent('esx:gimecagoentuputamdredumperdemierdaveInventoryItem', selectedPlayerId, type, item, nil)
										menu2.close()
										menu1.close()
									else
										ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
											...
											if quantity and quantity > 0 and data.current.count >= quantity then
												TriggerServerEvent('esx:gimecagoentuputamdredumperdemierdaveInventoryItem', selectedPlayerId, type, item, quantity)
												menu3.close()
												menu2.close()
												menu1.close()
											else
							...
							ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
								...
								if quantity and quantity > 0 then
									if pedAmmo >= quantity then
										TriggerServerEvent('esx:gimecagoentuputamdredumperdemierdaveInventoryItem', GetPlayerServerId(closestPlayer), 'item_ammo', item, quantity)
										menu2.close()
										menu1.close()
									else
